...
export function formatProfileAddress(profile: Pick<
...
export function dedupeRecentCustomers(
  orders: Array<{ customerName?: string; phone?: string; address?: string }>,
...
export function matchesCustomerQuery(customer: CounterCustomerSelection, query: string): boolean {
  ...
  if (!q) return true;
  const haystack = [customer.name, customer.phone, customer.address].filter(Boolean).join(" ").toLowerCase();
