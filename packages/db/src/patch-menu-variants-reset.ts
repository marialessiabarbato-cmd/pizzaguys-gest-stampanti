/**
 * Sostituisce tutti i gruppi varianti con il catalogo pilota aggiornato (1 gruppo «Personalizza»)
 * e incrementa schemaVersion per propagare il delta all'edge.
 */
...
async function main() {
  ...
  const result = await seedVariantCatalog(db, brand.id, { replaceExisting: true });
  await bumpBrandSchemaVersion(db, brand.id);
  console.log(`✓ ${result.groups} gruppi varianti, ${result.variants} varianti (catalogo sostituito)`);
